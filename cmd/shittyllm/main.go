...
import (
	...
	"github.com/rusik69/shittyllm/pkg/data"
	"github.com/rusik69/shittyllm/pkg/llm"
	"github.com/rusik69/shittyllm/pkg/tokenizer"
	"github.com/rusik69/shittyllm/pkg/training"
)
...
func main() {
	...
	if *showVersion {
		...
		fmt.Println("GitHub: https://github.com/rusik69/shittyllm")
		os.Exit(0)
	}
...
Programming languages provide tools for creating software applications.`
	...
	// Create larger model for better capacity and diversity
	model := llm.New(len(tokenizer.Vocab()), 128, 256, 3) // vocab_size, embed_size, hidden_size, num_layers
...
func generateText(prompt string, modelDir string, config *llm.GenerationConfig) error {
	...
	if err != nil {
		...
		model = llm.New(len(fallbackTokenizer.Vocab()), 128, 256, 3)
	}
